...
-   **视频播放**: 可见时自动播放，滑出时自动暂停，单击播放/暂停，长按功能弹窗，明确的资源释放。
-   **图片显示**: 支持视频/图片混合流，支持多张图片左右滑动切换，不支持手势放大缩小。
-   **懒加载**: 到达流末尾时自动分页和数据加载。
-   **交互占位符**: 预定义点赞、评论、分享功能的区域，已实现UI占位符。
-   **信息展示**: 视频标题、描述、可点击的话题/关联内容。
-   **音频指示器**: 视频播放时的音量指示器。
-   **进度条**: 底部进度条，暂停时常显，播放时隐藏。
-   **错误处理**: 优雅地处理网络和加载失败。
...
| 图片显示       | `photo_view` + `cached_network_image` + `PageView.builder` | `cached_network_image` 用于缓存，`photo_view` 用于展示单张图片，`PageView.builder` 用于多图滑动。
| 状态管理       | `Riverpod`                                       | 使用 `Riverpod` 进行响应式和可测试的状态管理。                     |
| 页面切换       | `PageView.builder` + `PageController`            | 用于垂直滚动流。
| 数据加载       | 自定义分页逻辑 + 网络层封装         | 实现数据获取的清晰职责分离。                                       |
| 进度条         | 自定义 `Slider` 实现                             | 结合 `better_player` 事件监听实现。
...
-   **`FeedMediaItemPage`**: 一个通用内容卡片，负责根据内容类型渲染视频或图片，并管理视频播放器的交互和进度条的显示。
-   **`FeedMediaVideoPlayer`**: 封装 `better_player` 逻辑，根据 `isActive` 状态处理播放、暂停和资源管理，并暴露 `BetterPlayerController`。
-   **`FeedMediaPhotoViewer`**: 处理图片显示，使用 `photo_view` 展示单张图片，并结合 `PageView.builder` 实现多图左右滑动。
-   **`FeedMediaOverlayUI`**: 负责显示视频标题、描述、话题、点赞/评论/分享按钮和音量指示器。
...
-   **自定义进度条**: 使用 Flutter `Slider` 结合 `better_player` 的 `addEventsListener` 来更新进度，提供了高度可定制的UI和更稳定的实现，避免了对 `better_player` 内部组件的依赖。
